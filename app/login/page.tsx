...
export default function Login() {
  ...
  function showError(msg: string) { setMessage(""); setToast(msg); }

  async function sendOtp() {
    setLoading(true); setMessage(""); setToast("");
    const res = await fetch("/api/auth/otp", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, mode: "login" }) });
    ...
    if (res.ok) setMessage("OTP sent. Check your email.");
      ...
      if (data?.signupRequired || res.status === 404) setTimeout(() => { window.location.href = `/signup?email=${encodeURIComponent(email)}`; }, 1400);
    }
  }

  async function verify() {
    setLoading(true); setMessage(""); setToast("");
    const res = await fetch("/api/auth/verify-otp", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, code, mode: "login" }) });
    const data = await res.json().catch(() => ({}));
    setLoading(false);
